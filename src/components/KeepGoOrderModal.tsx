...
import { X, ArrowLeft, Smartphone, Copy, Check, CheckCircle, Hash, Upload, Image as ImageIcon, ShieldCheck } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { useTranslation } from '../i18n';
import { KeepGoPlan, MAXICASH_NUMBER, MAXICASH_NAME, WHATSAPP_NUMBER, formatMaxicashNumber, planPriceCdf } from '../data/keepgo';
import { createKeepGoOrder } from '../services/keepgo';
import { submitPaymentProof, uploadProductImage } from '../services/database';
import { Transaction } from '../types';

interface KeepGoOrderModalProps {
  plan: KeepGoPlan;
  onClose: () => void;
  onOpenAuth: () => void;
}
...
export default function KeepGoOrderModal({ plan, onClose, onOpenAuth }: KeepGoOrderModalProps) {
  ...
  const whatsappLink = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
    `Bonjour, je viens de commander le forfait ${plan.name} ${plan.dataGb} Go (${plan.validityDays} jours) sur LDBusiness. Facture ${invoiceNumber}.`
  )}`;
  ...
  const startOrder = async () => {
    ...
    setCreating(true);
    setError('');
    const created = await createKeepGoOrder({
      buyerId: user.id,
    ...
    if (created) {
      ...
      setError(t('keepgo.orderError'));
    }
  };

  const submitProof = async () => {
    if (!txnId.trim() || !txn) {
      setError(t('keepgo.txnRequired'));
      return;
    }
    setSubmitting(true);
    setError('');
    ...
    else setError(t('keepgo.proofError'));
  };
      ...
      <div className="relative w-full max-w-lg bg-luxury-dark border-x border-t border-gold/20 rounded-t-3xl md:rounded-xl overflow-hidden flex flex-col h-[94vh] md:h-auto max-h-[92vh] shadow-2xl">
        <div className="p-5 border-b border-gold/10 flex items-center justify-between bg-luxury-light/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-playfair text-lg font-bold text-white tracking-tight">{t('keepgo.orderTitle')}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-5">
            ...
            <>
              <div className="bg-gold/5 border border-gold/20 rounded-lg p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-[10px] mt-1">
                    {plan.zone} · {plan.dataGb} Go · {plan.validityDays} jours
                  </p>
                </div>
              ...
              {!isAuthenticated ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-white font-bold text-lg mb-2">{t('keepgo.loginTitle')}</h3>
                  <p className="text-gray-400 text-sm mb-6 max-w-xs mx-auto">{t('keepgo.loginDesc')}</p>
                  <button
                    onClick={onOpenAuth}
                ...
                <>
                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('keepgo.name')} *</label>
                      <input type="text" value={customerInfo.name} onChange={e => setCustomerInfo({ ...customerInfo, name: e.target.value })} placeholder={t('keepgo.namePlaceholder')} className={inputCls} />
                    </div>
                    <div>
                      <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('keepgo.phone')} *</label>
                    ...
                    <div>
                      <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('keepgo.email')}</label>
                  ...
                  >
                    {creating ? t('keepgo.creating') : t('keepgo.continuePayment')}
                  </button>
                  <p className="text-[9px] text-gray-600 text-center">{t('keepgo.orderNotice')}</p>
                </>
              )}
            </>
          )}
          ...
          {step === 'payment' && txn && (
            <>
              <div className="bg-gold/5 border border-gold/20 rounded-lg p-4 flex items-center justify-between">
                <div>
                  <p className="text-gold text-xs font-bold uppercase tracking-widest">{plan.name}</p>
                  <p className="text-gray-400 text-[10px] mt-1">{t('keepgo.invoice')} {invoiceNumber}</p>
                </div>
                <div className="text-right">
                  <p className="text-white font-black text-lg">{price.toLocaleString()} CDF</p>
                </div>
              ...
              <div className="p-5 bg-black border-l-2 border-gold rounded-r-lg space-y-3">
                <div className="flex items-center gap-2 text-gold text-xs font-bold uppercase tracking-widest">
                  <Smartphone size={14} /> {t('keepgo.maxicashTitle')}
                </div>
                <p className="text-gray-400 text-xs leading-relaxed">{t('keepgo.payInstruction')}</p>
                <div className="flex items-center justify-between bg-luxury-dark border border-gold/20 rounded-sm px-4 py-3">
                  ...
                  <button onClick={copyNumber} className="flex items-center gap-1 px-3 py-2 border border-gold/30 text-gold text-[10px] uppercase tracking-widest rounded-sm hover:bg-gold/10 transition-all">
                    {copied ? <Check size={12} /> : <Copy size={12} />} {copied ? t('keepgo.copied') : t('keepgo.copy')}
                  </button>
                </div>
                <div className="flex justify-between text-xs pt-1">
                  <span className="text-gray-500">{t('keepgo.amountToPay')}</span>
              ...
              <div className="space-y-4 pt-2">
                <div>
                  <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('keepgo.txnIdLabel')} *</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t('keepgo.txnIdPlaceholder')}
                ...
                <div>
                  <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('keepgo.screenshot')}</label>
                  <label className="flex flex-col items-center justify-center h-24 bg-black border-2 border-dashed border-gold/20 rounded-sm cursor-pointer hover:border-gold/50 transition-all">
                    {screenshot ? (
                      ...
                      <div className="flex flex-col items-center gap-1.5 text-gray-500">
                        <Upload size={18} />
                        <span className="text-[10px]">{t('keepgo.touchToAdd')}</span>
                      </div>
                    )}
                ...
                >
                  {submitting ? (
                    <><div className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" /> {t('keepgo.sending')}</>
                  ) : (
                    <><ShieldCheck size={15} /> {t('keepgo.confirmPayment')}</>
                  )}
                </button>
                <p className="text-gray-600 text-[10px] text-center">{t('keepgo.verificationNotice')}</p>
              </div>
            </>
          )}
          ...
          {step === 'confirm' && (
            <div className="text-center py-6">
              ...
              <h3 className="font-playfair text-xl text-white font-bold mb-2">{t('keepgo.successTitle')}</h3>
              <p className="text-gray-400 text-sm mb-6">{t('keepgo.successDesc')}</p>

              <div className="bg-black border border-gold/20 rounded-lg p-4 text-left space-y-2 mb-6">
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500">{t('keepgo.receiptInvoice')}</span>
                  <span className="text-white font-mono font-bold">{invoiceNumber}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500">{t('keepgo.receiptPlan')}</span>
                  <span className="text-white font-bold">{plan.name} · {plan.dataGb} Go</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500">{t('keepgo.amountToPay')}</span>
                  <span className="text-gold font-black">{price.toLocaleString()} CDF</span>
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500">{t('keepgo.receiptStatus')}</span>
                  <span className="text-yellow-400">{t('keepgo.statusVerification')}</span>
                </div>
              </div>
