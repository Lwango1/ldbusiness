...
export default function LiveSection() {
  ...
  const categories = [t('live.categoryFashion'), t('live.categoryCraft'), t('live.categoryRunway'), t('live.categoryClothing'), t('live.categoryQA'), t('live.categoryEvent'), t('live.categoryAuto'), t('live.categoryElectronics'), t('live.categoryAppliances'), t('live.categoryHome'), t('live.categoryOther')];
  ...
  const handleStartLive = async (e?: React.MouseEvent | React.TouchEvent) => {
    ...
    if (!user) {
      setStartError(t('live.errorNotConnected'));
      return;
    }
    setStarting(true);
    try {
      ...
      else setStartError(t('live.errorCreate'));
    } catch (err: any) {
      console.error('startLive error:', err);
      setStartError(err.message || t('live.errorStart'));
    }
    setStarting(false);
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <span className="text-red-500 text-xs uppercase tracking-[0.3em] font-bold">{t('live.title')}</span>
          </div>
          <h2 className="font-playfair text-4xl md:text-5xl font-bold mt-2 mb-4">
            <span className="gold-shimmer">{t('live.showroom')}</span>
          </h2>
        ...
        <div className="text-center mb-12">
          <button
            onClick={async () => {
              if (!user) { setStartError(t('live.errorNotConnectedShort')); return; }
        ...
        {lives.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-gold/10 rounded-xl">
            ...
            <p className="text-gray-500 font-playfair italic text-lg mb-2">{t('live.noActive')}</p>
            <p className="text-gray-600 text-sm">{t('live.beFirst')}</p>
          </div>
        ) : (
          ...
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4" onClick={() => setShowSubscribe(false)}>
            ...
            <div className="relative bg-luxury-dark border border-gold/20 rounded-xl p-8 max-w-md w-full shadow-2xl text-center" onClick={e => e.stopPropagation()}>
              <Crown size={40} className="text-gold mx-auto mb-4" />
                <h3 className="text-white font-playfair text-xl font-bold mb-2">{t('live.restrictedTitle')}</h3>
              <p className="text-gray-400 text-sm mb-6">
                {t('live.restrictedDesc')}
              </p>
          ...
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
            ...
            <div className="relative bg-luxury-dark border border-gold/20 rounded-xl p-8 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-playfair text-lg text-white font-bold">{t('live.startLiveTitle')}</h2>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('live.formTitle')}</label>
                  <input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder={t('live.formTitlePlaceholder')} className="w-full px-4 py-3 bg-black border border-gold/10 rounded-sm text-white placeholder:text-gray-600 focus:border-gold outline-none text-sm" />
                </div>
                <div>
                  <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('live.formDescription')}</label>
                  <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={2} placeholder={t('live.formDescriptionPlaceholder')} className="w-full px-4 py-3 bg-black border border-gold/10 rounded-sm text-white placeholder:text-gray-600 focus:border-gold outline-none text-sm" />
                </div>
                <div>
                  <label className="text-[10px] text-gold/60 uppercase tracking-widest block mb-1">{t('live.formCategory')}</label>
                ...
                <button type="button" onClick={handleStartLive} disabled={!form.title || starting} className="w-full py-4 bg-gold text-black font-bold text-xs uppercase tracking-widest rounded-sm hover:bg-gold-light transition-all disabled:opacity-30 flex items-center justify-center gap-2 cursor-pointer select-none touch-manipulation">
                  {starting ? <><Loader size={16} className="animate-spin" /> {t('live.starting')}</> : <><Play size={16} /> {t('live.startButton')}</>}
                </button>
              </div>
            </div>
          </div>
