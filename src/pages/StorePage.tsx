  ...
  if (!seller) {
    return (
      <div className="min-h-screen pt-28 px-6 bg-luxury-black flex flex-col items-center justify-center">
        ...
        <p className="text-gray-400 font-playfair text-lg">{t('store.notFound')}</p>
        <button onClick={() => navigate('/')} className="mt-4 text-gold text-xs uppercase tracking-widest">{t('store.backHome')}</button>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="bg-luxury-dark border border-gold/10 rounded-xl p-8 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {seller.ownerName && (
                <p className="text-gray-500 text-sm">{t('store.managedBy')} {seller.ownerName}</p>
              )}
              {seller.description && (
                <p className="text-gray-400 text-sm mt-2 max-w-xl">{seller.description}</p>
              )}
              <p className="text-gray-600 text-xs mt-2">{products.length} {products.length > 1 ? t('store.products') : t('store.product')}</p>
            </div>
          </div>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 font-playfair italic">{t('store.empty')}</p>
          </div>
        ) : (
